...
data _⊢_ {n} where
  ...
  CaseF : {Γ : TEnv {n}} {L : Subset n} {e : Exp {n}} {U : ValU e} {f : ∀ l → l ∈ L → Ty {n}} {f-ok : ∀ l → (i : l ∈ L) → Γ ⊢ (f l i)}
          {scopecheck-t : ∀ l i n → n ∈`ᵀ (f l i) → n <ᴺ length Γ}
          → Γ ⊢ e ◁ Label L → Γ ⊢ CaseT e f
...
data _⊢_▷_ {n} where
  ...
  LabAEl : {Γ : TEnv {n}} {B : Ty {n}} {L L' : Subset n} {l : Fin n} {e : Exp {n}} {U : ValU e} {f : ∀ l → l ∈ L → Exp {n}}
           {scopecheck : ∀ l i n → n ∈` (f l i) → n <ᴺ length Γ}
  ...
  LabAEx : {Γ Γ' Θ : TEnv {n}} {D : Ty {n}} {L L' : Subset n} {sub : L' ⊆ L} {ok : Γ ⊢ D} {f : ∀ l → l ∈ L → Exp {n}} {f-t : ∀ l → l ∈ L' → Ty {n}} {eq : Θ ≡ (Γ' ++ ⟨ D , Γ ⟩)}
           {scopecheck : ∀ l i n → n ∈` (f l i) → n <ᴺ length Θ}
           {scopecheck-t : ∀ l i n → n ∈`ᵀ (f-t l i) → n <ᴺ length Θ}
           → Γ ⊢ D ≤ᵀ Label L
           → (∀ l → l ∈ L' → Γ ⊢ Single (LabI l) (Label ⁅ l ⁆) ≤ᵀ D)
           → (∀ l → (i : l ∈ L') → (Γ' ++ ⟨ (Single (LabI l)) D , Γ ⟩) ⊢ (f l (sub i)) ▷ (f-t l i))
           → Θ ⊢ CaseE (Var (length Γ')) f ▷ CaseT (Var (length Γ')) f-t
  LabAExDyn : {Γ Γ' Θ : TEnv {n}} {D : Ty {n}} {L : Subset n} {ok : Γ ⊢ D} {f : ∀ l → l ∈ L → Exp {n}} {f-t : ∀ l → l ∈ L → Ty {n}} {eq : Θ ≡ (Γ' ++ ⟨ D , Γ ⟩)}
           {scopecheck : ∀ l i n → n ∈` (f l i) → n <ᴺ length Θ}
           {scopecheck-t : ∀ l i n → n ∈`ᵀ (f-t l i) → n <ᴺ length Θ}
            → notSingle D
...
TyNf-lim⊂TyNf {n} {.UnitT} NfUnit = NfUnit
TyNf-lim⊂TyNf {n} {.(Label _)} NfLabel = NfLabel
TyNf-lim⊂TyNf {n} {.(Pi _ _)} NfPi = NfPi
TyNf-lim⊂TyNf {n} {.(Sigma _ _)} NfSigma = NfSigma
TyNf-lim⊂TyNf {n} {.(Single _ _)} (NfSingle{V = v}{tybB = tybB}) = NfSingle{V = v}{tybB = tybB}

TyNf-lim-uniqueness : {n : ℕ} {A : Ty {n}} → (a b : TyNf-lim A) → a ≡ b
TyNf-lim-uniqueness {n} {.UnitT} NfUnit NfUnit = refl
TyNf-lim-uniqueness {n} {.(Label _)} NfLabel NfLabel = refl
TyNf-lim-uniqueness {n} {.(Pi _ _)} NfPi NfPi = refl
...
data _⇨_ {n} where
  ...
  Cast-Top : {e : Exp {n}} {v : Val e} → Cast e Top Top ⇨ e
  Cast-Top-L : {e : Exp {n}} {v : Val e} {B : Ty {n}} → TyNf B → B ≢ Top × B ≢ Bot → Cast e Top B ⇨ Blame
  Cast-Top-R : {e : Exp {n}} {v : Val e} {A : Ty {n}} → TyNf A → A ≢ Top → Cast e A Top ⇨ e
